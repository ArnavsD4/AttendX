...
def teacher_dashboard():
    ...
    with tab1:
        t = "primary" if st.session_state.current_teacher_tab == 'take_attendance' else "secondary"
        if st.button('📷  Take Attendance', type=t, width='stretch'):
            st.session_state.current_teacher_tab = 'take_attendance'
            st.rerun()
    with tab2:
        t = "primary" if st.session_state.current_teacher_tab == 'manage_subjects' else "secondary"
        if st.button('📚  Manage Subjects', type=t, width='stretch'):
            st.session_state.current_teacher_tab = 'manage_subjects'
            st.rerun()
    with tab3:
        t = "primary" if st.session_state.current_teacher_tab == 'attendance_records' else "secondary"
        if st.button('📋  Attendance Records', type=t, width='stretch'):
...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning('No subjects yet. Create one in Manage Subjects.')
        return
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn(sub=sub):
                if st.button(f"🔗 Share: {sub['name']}", key=f"share_{sub['subject_code']}"):
